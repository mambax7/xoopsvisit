...
class XoopsvisitMenuHandler {
     ...
     * Constructor
     */
    function XoopsvisitMenuHandler() {
        global $xoopsModule;
    ...
    function render( $currentoption = 1, $display = true ) {
		...
		*/
        ...
        $menu = "<style type=\"text/css\" media=\"screen\">@import \"" . XOOPS_URL . "/modules/thadmin/css/menu.css\";</style>";
        ...
        //$menu .= "<a class='nobutton_mod' href='" . XOOPS_URL . "/modules/system/admin.php?fct=preferences&amp;op=showmod&amp;mod=" . $this->_obj->getVar( 'mid' ) . "'>" . _MD_AM_PREF . "</a>";
        ...
        foreach ( $this->_menutabs as $k => $v ) {
            $menu .= "<li id='" . $menuItems[$i] . "'><a href='" . XOOPS_URL . '/modules/' . $this->_obj->getVar('dirname', 'e') . '/'.$k."'><span>$v</span></a></li>\n";
            $i++;
        }
        $menu .= "</ul>\n</div>\n";
        if ( $this->_header ) {
